...
bne  +
b    .custom_cc              // if so, jump to our special code to calculate its length unpacked
+
...
//-----------------------
                             // READ VALUE
ldr  r0,=#0x2014380
ldrh r1,[r0,#0]              // gets enemy id from where it was stored
...
pop  {r1-r2}
bl   custom_strlen           // count the length of our special string, store its length in r2
b    .main_loop_next         // now jump back to the part of the main loop that increments and suc
...
+
                             // WRITE VALUE
ldr  r2,=#0x2014320
ldrh r1,[r2,#0]              // reads value from its initial place


ldr  r2,=#0x2014380
strh r1,[r2,#0]              // copies it into r1


ldr  r0,=#0x8D0829C          // load r0 with the base address of our custom text array in ROM
...
.cc_of_specific_guy:
push {r1-r2}
lsr  r1,r4,#5                // the multiples of 20 contain the information on which character it is
cmp  r0,#0x1F
bne  +
add  r1,#0x8                 // if customcode is 0x1F and not 0x1E, other series of 8 characters
+
ldr  r2,=#0x14A
bl   extra_hacks.has_elision
add  r2,r2,r0
mov  r1,r2
lsl  r1,r1,#1
lsl  r0,r1,#1
...
mov  r0,#0x1F
and  r0,r4                   // load the first 5 bits of the custom control code
...
cmp  r0,#0x06                // check for 0xEF06, which will print l'accord de genre
bne +
b  .cc_en_articles
+


cmp  r0,#0x07                // check for 0xEF07, which will print du/de la/des/ de l'
bne +
b  .cc_en_articles
+


cmp  r0,#0x08                // check for 0xEF08, which will print nt
bne +
b  .cc_plural_verb
+
...
cmp  r0,#0x16                // check for 0xEF16, which will print l'accord
bne +
b  .cc_it_articles
+


cmp  r0,#0x17                // check for 0xEF17, which will print COD
bne +
b  .cc_it_articles
+


cmp  r0,#0x1D                // check for 0xEF10, which will store an enemy ID or use the last one stored
bne +
b  .cc_of_previous_guy
+


cmp  r0,#0x1E                // check for 0xEF1E, which will print "de" for any specific party member
bne  +
b  .cc_of_specific_guy
+
cmp  r0,#0x1F                // check for 0xEF1F, which will print "de" for any specific party member
bne  +
b  .cc_of_specific_guy
+
...
+
ldr  r2,=#0x149
cmp  r0,r2                            // If the actor id is > 0x149, it's Salsa.
blt  +


ldr  r1,=#0x200439A               // Character data address
mov  r0,r1                        // r0 now has the address of Salsa
pop  {r1-r2}
...
cmp  r7,#0xEF
bne  +
b  .check_custom_cc          // if this is a custom CC, go do stuff, else do old code and leave
+
...
mov  r2,r0                   // r2 will be an offset into the extra item data slot
ldr  r0,=#0x2014324          // this is where the current item # will be saved by another hack
cmp  r7,#0x20                // check if this is EF 3x
blt  +
...
.ecc_of_specific_guy:
push {r1-r2}
lsr  r1,r7,#5                // the multiples of 20 contain the information on which character it is
cmp  r0,#0x1F
bne  +
add  r1,#0x8                  // if customcode is 0x1F and not 0x1E, other series of 8 characters
+
...
mov  r0,#0x1F                // load only the first 5 bits of it
and  r0,r7
...
cmp  r0,#0x1D                // check for 0xEF10, which will store an enemy ID or use the last one stored
bne +
b  .ecc_of_previous_guy
+


cmp  r0,#0x1E                // check for 0xEF1E, which will print "de" for any specific party member
bne +
b  .ecc_of_specific_guy
+
cmp  r0,#0x1F                // check for 0xEF1F, which will print "de" for any specific party member
bne +
b  .ecc_of_specific_guy
...
+
ldr  r2,=#0x149
cmp  r0,r2                            // If the actor id is > 0x149, it's going to be Salsa
blt  +
